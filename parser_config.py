...
[b]Instruction Pointer:[/b] Stored in EIP before the call.
[b]Return Value:[/b] Stored in EAX.
[b]Stack Frame Layout:[/b]
    - [ESP] → Return address (saved EIP).
    - [ESP+4] → First argument.
    - Arguments pushed [b]right-to-left[/b].
    - The caller cleans up the stack after the function returns.
[b]Stack Pointers:[/b] Uses ESP as the stack pointer, and EBP can be used as a frame pointer if enabled.

[green]
ESP ->  +----------------------+
        | Return Address (EIP) |  <-- ESP (Contient l'adresse suivante aprés la fonction appelante)
        +----------------------+
        | Argument 1           |  <-- ESP + 4
        +----------------------+
        | ...                  |
...
- [b]Stack Frame Layout[/b]
    - [ESP] → Return address (saved EIP).
    - First two arguments passed in ECX and EDX, remaining ones pushed on the stack.
    - The callee cleans up the stack.
- [b]Stack Pointers[/b] Uses ESP, and optionally EBP for local variables.
...
thiscall_calling_convention = """
- [b]Instruction Pointer[/b] Stored in EIP before the call.
- [b]Return Value[/b] Stored in EAX.
- [b]Stack Frame Layout[/b]
    - ECX stores the this pointer.
    - Other arguments pushed [b]right-to-left[/b] on the stack.
    - The callee cleans up the stack.
- [b]Stack Pointers[/b] Uses ESP, and optionally EBP for stack frames."""

arm32_calling_convention = """
- [b]Instruction Pointer[/b] Stored in LR (Link Register) before the call.
- [b]Return Value[/b] Stored in R0 (primary) and R1 (secondary if needed).
- [b]Stack Frame Layout[/b]
    - [SP] → Return address (saved LR, if pushed).
    - [SP+4] → Saved frame pointer (FP, if used).
    - [SP+8] → First spilled argument (if needed).
    - First [b]four arguments[/b] passed in registers R0-R3.
    - Additional arguments are stored on the stack.
    - Stack must be [b]8-byte aligned[/b] before the call.
- [b]Stack Pointers[/b] Uses SP (stack pointer) and optionally FP (frame pointer, usually R7 or R11 in some compilers).

[b][green]SP ->   +----------------------+
        | Return Address (LR)  |  <-- SP (Contient l'adresse suivante aprés la fonction appelante)
        +----------------------+
        | Saved FP (if used)   |  <-- SP + 4
        +----------------------+
        | Argument 5+          |  <-- SP + 8
        +----------------------+
...
- [b]Instruction Pointer[/b] Stored in RIP before the call.
- [b]Return Value[/b] Stored in RAX (scalars, integers, pointers).
- [b]Stack Frame Layout[/b]
    - [RSP] → Return address (saved RIP).
    - Shadow space (32 bytes) reserved for the callee.
    - First four arguments in RCX, RDX, R8, R9, others passed on the stack.
    - Stack must be 16-byte aligned before the call.
- [b]Stack Pointers[/b] Uses RSP (stack pointer); RBP can be used as a frame pointer.

[green]
RSP ->  +----------------------+
        | Return Address (RIP) |  <-- RSP (Contient l'adresse suivante aprés la fonction appelante)
        +----------------------+
        | Shadow Space (32B)   |  <-- RSP + 8
        | (for callees to use) |
        +----------------------+
        | Argument 5+          |  <-- RSP + 40
        +----------------------+
...
amd64_calling_convention = """
- [b]Instruction Pointer[/b] Stored in RIP before the call.
- [b]Return Value[/b] Stored in RAX (primary) and RDX (secondary if needed).
- [b]Stack Frame Layout[/b]
    - [RSP] → Return address (saved RIP).
    - [RSP+8] → Old RBP (saved frame pointer).
    - [RSP+16] → First spilled argument (if needed).
    - Arguments passed in registers: RDI, RSI, RDX, RCX, R8, R9.
    - Additional arguments pushed right-to-left on the stack.
    - Stack must be 16-byte aligned before the call.
- [b]Stack Pointers[/b] Uses RSP (stack pointer) and RBP (frame pointer if used).

[green]
RSP ->  +----------------------+
        | Return Address (RIP) |  <-- RSP (Contient l'adresse suivante aprés la fonction appelante)
        +----------------------+
        | Old RBP (frame ptr)  |  <-- RSP + 8
        +----------------------+
        | Argument 7+          |  <-- RSP + 16
        +----------------------+
[/green]
